...
class Scipy:
    ...
    def eval_func(
        ...
        def _tf_eval(x: tf.Tensor) -> Tuple[tf.Tensor, tf.Tensor]:
            ...
            unused_variables = [variables[i].name for i, grad in enumerate(grads) if grad is None]
            if unused_variables:
                raise ValueError(
                    "Some variables does not have a gradient, and appear unused in / not connected"
                    f" to the loss closure: {unused_variables}."
                )
