...
from typing import Any, Callable, DefaultDict, Iterator, Mapping, Set, Tuple, Type, TypeVar
...
from gpflow.models import GPR, SGPR, SVGP, VGP, GPModel, on_data_change, training_loss_closure
...
_MODEL_FACTORIES: Mapping[_CreateModel, Mapping[str, Any]] = {}
...
def model_factory(*flags: str, atol=_DEFAULT_ATOL, rtol=_DEFAULT_RTOL) -> Callable[[_C], _C]:
...
def _create_model(request) -> _CreateModel:
...
def _extend_data(
    ...
    def iterate() -> Iterator[int]:
        for i in range(n_iterations):
            X.assign(tf.concat([X, rng.random((1, n_features))], axis=0))
            Y.assign(_f(X))
            yield i
